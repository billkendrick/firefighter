...
void read_settings(void) {
  ...
  if (dstats == 1 /* operation complete (no errors) */) {
    ...
    if (OS.dcb.dstats == 1 /* operation complete (no errors) */) {
      high_score = (
        (app_key_data.read.value[0] << 24) |
        (app_key_data.read.value[1] << 16) |
        (app_key_data.read.value[2] << 8) |
        (app_key_data.read.value[3])
      );

      main_stick = app_key_data.read.value[5];
      level = app_key_data.read.value[6];
...
unsigned char write_settings(void) {
  ...
  if (dstats == 1 /* operation complete (no errors) */) {
    ...
    app_key_data.write.value[4] = main_stick;
    app_key_data.write.value[5] = level;
    ...
    OS.dcb.daux = 6; /* How many bytes of data (see above) */
